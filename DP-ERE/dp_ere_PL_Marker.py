...
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), './transformers')))
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                self.global_predicted_ners[(l_idx, n)] = valid_ners
                ...
                for x in sentence_relations:
                    ...
                    self.golden_labels_withner.add(((l_idx, n), (x[0], x[1], std_entity_labels.get((x[0], x[1]), 'NIL')), (x[2], x[3], std_entity_labels.get((x[2], x[3]), 'NIL')), x[4]))
                    if x[4] in self.sym_labels[1:]:
                        self.golden_labels.add(((l_idx, n), (x[2], x[3]), (x[0], x[1]), x[4]))
                        self.golden_labels_withner.add(((l_idx, n), (x[2], x[3], std_entity_labels.get((x[2], x[3]), 'NIL')), (x[0], x[1], std_entity_labels.get((x[0], x[1]), 'NIL')), x[4]))

                entities = list(valid_ners)
                ...
                for sub_idx, sub in enumerate(entities):
                    ...
                    if sub[0] < 10000:
                        ...
                        try:
                            ...
                            logger.warning(f"Subword index out of bounds for NER span in doc {l_idx}, sent {n}: ({sub[0]}, {sub[1]})")
                    ...
                    for obj_idx, (start, end, obj_label) in enumerate(valid_ners):
                        ...
                        try:
                            ...
                            logger.warning(f"Subword index out of bounds for relation span in doc {l_idx}, sent {n}: ({start}, {end})")
                            continue
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    if not args.eval_unidirect:
        for example_index, pair_dict in sorted(scores.items(), key=lambda x: x[0]):
            ...
            def is_overlap(m1, m2):
                if m2[0] <= m1[0] <= m2[1] or m1[0] <= m2[0] <= m1[1]:
                    return True
                return False
            ...
            for item in no_overlap:
                ...
                if pred_label in sym_labels:
                    ...
                    if (example_index, m1, m2, pred_label) in golden_labels or (example_index, m2, m1, pred_label) in golden_labels:
                        ...
                        wrong_predictions.append((example_index, m1, m2, pred_label))
                else:
        ...
        for example_index, pair_dict in sorted(scores.items(), key=lambda x: x[0]):
            ...
            for k1, (v1, v2_ner_label) in pair_dict.items():
                ...
                pred_label = np.argmax(v1)
                pred_score = v1[pred_label]
                if pred_label > 0 and pred_label < num_label and pred_score >= -5.0:
    ...
    # Group and print wrong predictions
    wrong_by_type = defaultdict(list)
    for wp in wrong_predictions:
        example_index, m1, m2, pred_label = wp
        words = eval_dataset.sentences_dict[example_index]
        m1_start = max(0, min(m1[0], len(words) - 1))
        m1_end = max(0, min(m1[1], len(words) - 1))
        m2_start = max(0, min(m2[0], len(words) - 1))
        m2_end = max(0, min(m2[1], len(words) - 1))
        entity1 = ' '.join(words[m1_start:m1_end + 1]) or '<empty>'
        entity2 = ' '.join(words[m2_start:m2_end + 1]) or '<empty>'
        if entity1 == '<empty>' or entity2 == '<empty>':
            continue
        sentence = ' '.join(words)

        true_relation = None
        for label in label_list:
            if (example_index, m1, m2, label) in golden_labels:
                true_relation = f"{label} (forward)"
                break
            elif (example_index, m2, m1, label) in golden_labels:
                true_relation = f"{label} (reverse)"
                break
        if true_relation is None:
            true_relation = "no relation"
    ...
    for rel_type, examples in wrong_by_type.items():
        print(f"\nRelation Type: {rel_type}")
        random.shuffle(examples)
        for ex in examples[:max_per_type]:
            sentence, entity1, entity2, pred_label, true_relation = ex
            print(f"Sentence: {sentence}")
            print(f"Entity 1: {entity1}")
            print(f"Entity 2: {entity2}")
            print(f"Predicted: {entity1} - {pred_label} -> {entity2}")
            print(f"True: {true_relation}")
            print()
            total_printed += 1
            if total_printed >= max_total:
                break
        if total_printed >= max_total:
            break
    ...
    r = cor / tot_recall
    rel_f1 = 2 * (p * r) / (p + r) if cor > 0 else 0.0
    ...
    logger.info("Rel F1: %.4f", rel_f1)
    logger.info("Ner_F1: %.4f", ner_f1)

    results = {'f1': rel_f1, 'f1_with_ner': rel_plus_f1, 'ner_f1': ner_f1}
